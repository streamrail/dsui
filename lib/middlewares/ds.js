...
class DS {
    ...
    async getNamespaces(){
        ...
        const [entities] = await this._datastore.runQuery(query);
        const namespaces = entities.map(entity => entity[this._datastore.KEY].name).filter(ns => typeof ns !== 'undefined');
    ...
    async getEntity(key) {
        let datastoreKey = this.deserializeKey(key);
        let result = await this._datastore.get(datastoreKey)
        if (typeof result[0] === 'undefined') {
            datastoreKey = this.deserializeKey(key, true);
            result = await this._datastore.get(datastoreKey, true);
        }
        return result[0];
    }

    async deleteEntities(keys) {
        const entities = await Promise.all(keys.map(key => this.getEntity(key)));
        const datastoreKeys = entities.filter(x => !!x).map(entity => entity[this._datastore.KEY])
        const groups = _.chunk(datastoreKeys, DELETE_CHUNK_SIZE);
        for (let group of groups) {
            await this.deleteDatstoreKeys(group);
        }
    }

    async deleteDatstoreKeys(datastoreKeys){
        const transaction = this._datastore.transaction();
        await transaction.run();
        transaction.delete(datastoreKeys);
        try {
